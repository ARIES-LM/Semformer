...
def generate_and_save(n_train, n_test, degSource, pathLen, numNodes, reverse=False):
    ...
    """
    file = open('datasets/graphs/' + 'deg_' + str(degSource) + '_path_' + str(pathLen) + '_nodes_' + str(
        numNodes) + '_train_' +
          str(n_train) + '.txt', 'w')
    ...
    file = open('datasets/graphs/' + 'deg_' + str(degSource) + '_path_' + str(pathLen) + '_nodes_' +
                str(numNodes) + '_test_' +
                 str(n_test) + '.txt', 'w')
...
class ZGraphs(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if model_args.fullenc:
            input_ids_enc = self.tokenized[idx].clone()
        else:
...
if __name__ == '__main__':
    ...
    path_len = 5 # TODO : change pathlen
